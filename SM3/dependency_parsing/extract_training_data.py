...
dep_relations = ['tmod', 'vmod', 'csubjpass', 'rcmod', 'ccomp', 'poss',
                 'parataxis', 'appos', 'dep', 'iobj', 'pobj', 'mwe',
                 'quantmod', 'acomp', 'number', 'csubj', 'root',
                 'auxpass', 'prep', 'mark', 'expl', 'cc', 'npadvmod',
                 'prt', 'nsubj', 'advmod', 'conj', 'advcl', 'punct',
                 'aux', 'pcomp', 'discourse', 'nsubjpass', 'predet',
                 'cop', 'possessive', 'nn', 'xcomp', 'preconj', 'num',
...
class FeatureExtractor(object):
    ...
    def get_next_token(self, ds, words, pos, i):
        ind = ds[-i]
        word = words[ind]

        if word is None:
            return self.word_vocab['<ROOT>']
        if pos[ind] in ['CD', 'NNP', 'NNPS']:
            return self.word_vocab['<{}>'.format(pos[ind] if pos[ind] != 'NNPS'
                                                 else 'NNP')]
        word = word.lower()
        if not self.word_vocab.get(word):
            return self.word_vocab['<UNK>']

        return self.word_vocab[word]

    def get_input_representation(self, words, pos, state):
        null_ind = self.word_vocab['<NULL>']

        inp = []
        # stack tokens
        for i in range(1, 4):
            if len(state.stack) >= i:
                inp.append(self.get_next_token(state.stack, words, pos, i))
            else:
                while len(inp) != 3:
                    inp.append(null_ind)
                break
        ...
        for i in range(1, 4):
            if len(state.buffer) >= i:
                inp.append(self.get_next_token(state.buffer, words, pos, i))
            else:
                while len(inp) != 6:
                    inp.insert(3, null_ind)
                break
    ...
    def get_output_representation(self, output_pair):
        labels = []
        labels.append(self.output_labels[output_pair])

        return keras.utils.to_categorical(labels, num_classes=91)
